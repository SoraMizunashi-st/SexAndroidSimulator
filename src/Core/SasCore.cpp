...
int SAS::SasCore::MainLoop()
{
    ...
    while( true )
    {
        ...
        // ---------------------------------------------------------------------------------------------------------------------------------//
        // LifeTime Layer
        if( 10 < this->android->getDesireEat() ){ this->android->setCost( SAS::E_COST::LIFE_TIME , -2 );}
        else{this->android->setCost( SAS::E_COST::LIFE_TIME , 0);}

        // Wallet Layer
        if( 10 < this->android->getDesireSleep() && 10 < this->android->getDesireSexual() ){this->android->setCost( SAS::E_COST::WALLET , -30 );}
        else if(10 < this->android->getDesireSleep()){this->android->setCost( SAS::E_COST::WALLET , -10);}
        else if( 10 < this->android->getDesireSexual() ){this->android->setCost( SAS::E_COST::WALLET , -20);}
        else{this->android->setCost( SAS::E_COST::WALLET , 0);}

        // ---------------------------------------------------------------------------------------------------------------------------------//
        // ActionChain | Collect Initial Cost
        // ---------------------------------------------------------------------------------------------------------------------------------//
        this->android->AddLifeTime(this->android->getCost( SAS::E_COST::LIFE_TIME)).AddWallet(this->android->getCost( SAS::E_COST::WALLET));
        ...
        // ---------------------------------------------------------------------------------------------------------------------------------//
        if( 10 < this->android->getDesireEat()    ){this->android->AddDesireEat(-10);   }else{this->android->AddDesireEat(5);}
        if( 10 < this->android->getDesireSleep()  ){this->android->AddDesireSleep(-10); }else{this->android->AddDesireSleep(4);}
        if( 10 < this->android->getDesireSexual() ){this->android->AddDesireSexual(-10);}else{this->android->AddDesireSexual(3);}
...
SAS::SasCore& SAS::SasCore::Result()
{
    ...
    std::cout << " Result | Actions Cost [Initial Cost] -> LifeTime : "
              ...
              << this->android->getCost( SAS::E_COST::LIFE_TIME) << " Vita / Money : "
              << this->android->getCost( SAS::E_COST::WALLET)   << " Pico" << std::endl;
